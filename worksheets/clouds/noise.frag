...
void main() {
  vec3 p1 = vec3(fragPosition * scale, height);
  float n1 = worley(p1, 1.0).x;

  vec3 p2 = vec3(fragPosition * scale * 2.0, height);
  float n2 = worley(p2, 1.0).x;

  vec3 p3 = vec3(fragPosition * scale * 3.0, height);
  float n3 = worley(p3, 1.0).x;

  vec3 p4 = vec3(fragPosition * scale * 4.0, height);
  float n4 = worley(p4, 1.0).x;

  vec4 color = vec4(n1, n2, n3, n4);
